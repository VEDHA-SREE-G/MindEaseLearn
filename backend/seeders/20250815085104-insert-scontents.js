...
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.bulkInsert('scontents', [
      {
        title: "Micro Controller",
        body: `A microcontroller (MC, uC, or μC) or microcontroller unit (MCU) is a small computer on a single integrated circuit. 
...
By reducing the size and cost compared to a design that uses a separate microprocessor, memory, and input/output devices, microcontrollers make digital control of more devices and processes practical. 
...
They generally have the ability to retain functionality while waiting for an event such as a button press or other interrupt; power consumption while sleeping (with the CPU clock and most peripherals off) may be just nanowatts, making many of them well suited for long lasting battery applications. 
Other microcontrollers may serve performance-critical roles, where they may need to act more like a digital signal processor (DSP), with higher clock speeds and power consumption.`,
  ...
  async down(queryInterface, Sequelize) {
    await queryInterface.bulkDelete('scontents', { title: "Micro Controller" }, {});
  }
};
